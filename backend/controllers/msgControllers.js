...
export const getAllChats = async (id) => {
  ...
  try {
    ...
    resp = {
      success: false,
      data: null,
      message: "not able to send all-chats",
    };
  }
...
export const getAllMessages = async (selfId, peerId) => {
  ...
  try {
    ...
    resp = {
      success: false,
      data: [],
      message: "error while finding user's user_contacts",
    };
    return resp;
  }
  ...
  while (lastMsgId !== "null" && lastMsgId !== null) {
    ...
    try {
      ...
      resp = {
        success: false,
        data: [],
        message: "error while collecting chat's messages",
      };
      return resp;
    }
...
export const storeOneMessage = async (data) => {
  ...
  try {
    ...
    resp = {
      success: false,
      data: [],
      message: "failed while getting last-message-id",
    };
    return resp;
  }
  ...
  if (last_msg_id === "null") {
    ...
    try {
      ...
      resp = {
        success: false,
        data: [],
        message: "error while getting the last-message using its _id",
      };
      return resp;
    }
  }
  ...
  const new_msg = new Message({
    _id:
      concatenateLexicographically(data.sender_id, data.receiver_id) +
      (last_msg.message_no + 1),
...
export const updateUnreadMessages = async (req, res) => {
  ...
  try {
    updatedUserFriend = await User.findOneAndUpdate(
      { _id: userId, "user_contacts._id": contactId }, // Match the user and specific contact
      {
        $inc: {
          "user_contacts.$.unread_messages": 1, // Update only the chat_id
        },
      },
      { new: true } // Return the updated document
    );
    ...
    res.json({
      success: false,
      message: "failed to update the user's unread_messages ",
    });
  }
  res.json({
    success: true,
    message: "successfully updated the user's unread_messages ",
  });
};
